...
// ── Dept & category options for filter dropdowns ──────────────────────────────
$deptOptions = [];
...
<head>
  ...
  <title>All Tickets | UniKL Help Desk – Maintenance</title>
    ...
    <form method="get" class="adv-filter-bar">
      ...
      <div class="adv-filter-group"><label>Category</label>
        <select name="category" class="adv-filter-select">
          ...
          ?>
          <option value="<?php echo (int)$cat['category_id']; ?>" <?php echo $filterCategory==(string)$cat['category_id']?'selected':''; ?>><?php echo htmlspecialchars($catShort); ?></option>
          <?php endforeach; ?>
        </select>
      </div>
    ...
    <div class="active-chips">
      ...
      <?php if ($filterCategory):
        ...
        foreach ($categoryOptions as $cat) {
          if ((string)$cat['category_id'] === $filterCategory) {
            $activeCatName = strpos($cat['category_name'], ' / ') !== false ? trim(substr($cat['category_name'], strpos($cat['category_name'], ' / ') + 3)) : $cat['category_name'];
            break;
          }
        }
