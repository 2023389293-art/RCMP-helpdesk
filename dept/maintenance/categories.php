<?php
// dept/maintenance/categories.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        $name   = $suffix !== '' ? 'Maintenance Department / ' . $suffix : '';
        if (empty($suffix)) {
    ...
    elseif ($action === 'edit') {
        ...
        $suffix = trim($_POST['category_name'] ?? '');
        $suffix = trim(preg_replace('/^(Maintenance Department\s*\/\s*)+/i', '', $suffix));
        $name   = $suffix !== '' ? 'Maintenance Department / ' . $suffix : '';
        if (!$id || empty($suffix)) {
...
<head>
  ...
  <title>Categories | UniKL Help Desk – Maintenance</title>
  ...
  <style>
    ...
    .alert-success{background:#ECFDF5;color:#065F46;border:1px solid rgba(14,116,144,.15)}
    .alert-success svg{stroke:#0E7490}
...
@media(max-width:900px){
  ...
  .page-grid > div:last-child{order:-1}
    ...
    .search-input{padding:8px 12px 8px 32px;font-size:13px;border-radius:8px;border:1.5px solid var(--g200);background:white;color:var(--g900);width:200px;max-width:100%;outline:none;transition:border-color .15s}
    ...
    .usage-some{background:#CFFAFE;color:#0E7490}
    ...
    .form-input:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(14,116,144,.08)}
    ...
    .btn-primary:hover{background:#155E75;transform:translateY(-1px)}
    ...
    .btn-stay-modal:hover{background:#155E75}
        ...
        <div class="tbl-card">
          <div class="card-header">
            <div class="card-header-left">
              ...
              <div>
                <div class="card-header-title">Maintenance Categories</div>
            ...
            <table id="cat-table">
              ...
              <tbody id="cat-tbody">
                ...
                <tr data-name="<?php echo strtolower(htmlspecialchars($cat['category_name'])); ?>">
                  ...
                  <td>
                    <div class="cat-name"><?php echo htmlspecialchars(preg_replace('/^Maintenance Department\s*\/\s*/i', '', $cat['category_name'])); ?></div>
                  </td>
                  <td>
...
function startEdit(id, fullName) {
  var suffix = fullName.replace(/^(Maintenance Department\s*\/\s*)+/i, '').trim();
...
function runPending() {
  if (typeof pendingFn === 'function') {
    var fn = pendingFn; pendingFn = null; fn();
  }
}
