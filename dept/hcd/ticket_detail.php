...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $ticket) {
    ...
    if ($action === 'update') {
        ...
        } elseif (!in_array($newStatus, $allowedSta)) {
        ...
        $freshStmt->bind_param("si", $ticketId, $deptId);
        $freshStmt->execute();
        ...
        if ($slaResetNeeded) {
            ...
            if ($upd->execute()) {
                ...
                if ($priChanged || $statChanged) {
                    ...
                    $logStaffId   = (int)($staffId ?? $_SESSION['staff_id'] ?? 0);
                    $logStaffName = $staffName ?? $_SESSION['staff_name'] ?? 'Unknown';
                    $logStmt = $conn->prepare(
                        "INSERT INTO ticket_logs
                         (ticket_id,changed_by_id,changed_by,field_changed,old_priority,new_priority,old_status,new_status)
                         VALUES (?,?,?,?,?,?,?,?)"
                    );
                    if ($logStmt) {
                        $logStmt->bind_param("sissssss",
                            $ticketId, $logStaffId, $logStaffName,
                            $fc, $oldPriority, $newPriority, $oldStatus, $newStatus
                        );
                        $logStmt->execute();
                        $logStmt->close();
                    }
                ...
                $_SESSION['flash_error'] = 'Failed to update. Please try again.';
            }
            $upd->close();
        }
        header('Location: ticket_detail.php?id=' . urlencode($ticketId) . '&tab=detail&from=' . $backUrlEncoded); exit;
...
if ($ticket) {
    $lg = $conn->prepare(
        "SELECT log_id,changed_by,field_changed,old_priority,new_priority,old_status,new_status,changed_at
         FROM ticket_logs WHERE ticket_id=? ORDER BY changed_at DESC"
    );
...
<head>
  ...
  <style>
    /* ── HCD purple accent overrides ── */
    .td-tab-btn.active { border-bottom-color: #7C3AED; color: #7C3AED; }
    .td-tab-btn:hover  { border-bottom-color: #DDD6FE; }
    .td-tab-btn.active .td-tab-badge { background: #7C3AED; color: white; }
    .ths-icon { background: #7C3AED; }
    .assigned-pill { background: #F5F3FF; border-color: #DDD6FE; }
    .assigned-avatar { background: linear-gradient(135deg,#4C1D95,#7C3AED); }
    .assigned-name { color: #3B0764; }
    .assigned-role-tag { color: #7C3AED; background: #EDE9FE; }
    .assigned-role-tag::before { background: #A78BFA; }
    .reassign-btn { background: #7C3AED; }
    .reassign-btn:hover { background: #6D28D9; }
    .reassign-select:focus { border-color: #7C3AED; }
    .update-header-icon { background: #F5F3FF !important; }
    .update-header-icon svg { stroke: #7C3AED !important; }
    .btn-update-save { background: linear-gradient(135deg,#4C1D95,#7C3AED) !important; }
    .conv-header { background: #7C3AED !important; }
    .conv-header-icon svg { stroke: #DDD6FE !important; }
    .conv-input-row:focus-within { border-color: #7C3AED !important; box-shadow: 0 0 0 3px rgba(124,58,237,.07) !important; }
    .conv-attach-label:hover { color: #7C3AED !important; }
    .conv-send-btn { background: linear-gradient(135deg,#4C1D95,#7C3AED) !important; box-shadow: 0 2px 8px rgba(124,58,237,.3) !important; }
    .feedback-card-header { background: linear-gradient(135deg,#2E1065 0%,#4C1D95 60%,#7C3AED 100%) !important; }
    .btn-modal-confirm { background: #7C3AED !important; }
    .btn-modal-confirm:hover { background: #6D28D9 !important; }
    .td-tab-dot { background: #7C3AED; }
  </style>
</head>
<body>
...
<?php require_once __DIR__ . '/_layout.php'; ?>
  ...
  <div class="td-tab-bar" role="tablist">
    ...
    <a href="?id=<?php echo urlencode($ticketId); ?>&tab=conversation&from=<?php echo $backUrlEncoded; ?>"
       class="td-tab-btn <?php echo $activeTab==='conversation'?'active':''; ?>" role="tab">
    ...
    <div class="detail-tab-grid">
        ...
        <div class="td-card ticket-info-card">
          <div class="td-card-header">
            <div class="td-card-header-icon" style="background:#F5F3FF">
              <svg viewBox="0 0 24 24" style="stroke:#7C3AED"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
            </div>
            <div>
      ...
      <div class="detail-right">
        ...
        <div class="td-card">
          <div class="td-card-header">
            <div class="td-card-header-icon" style="background:#F5F3FF">
              <svg viewBox="0 0 24 24" style="stroke:#7C3AED"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
            </div>
            <div>
    ...
    <div class="conv-card">
      ...
      <div class="conv-messages" id="chatMessages">
        ...
        ?>
          ...
          <div class="date-sep"><span><?php echo $msgDate === date('d M Y') ? 'Today' : $msgDate; ?></span></div>
          <?php endif; ?>
  ...
  <div class="td-panel <?php echo $activeTab==='history'?'active':''; ?>">
    <div class="history-card">
      ...
      <div class="history-empty">
        <div class="history-empty-icon"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><polyline points="12,8 12,12 14,14"/></svg></div>
          ...
          <div class="tl-body">
            ...
            <div class="tl-changes" style="margin-top:5px">
              ...
              <div class="tl-row">
                ...
                <span class="tl-chip-name" style="background:#F5F3FF;color:#6D28D9;"><?php echo htmlspecialchars($log['changed_by']); ?></span>
              </div>
      ...
      <div class="feedback-card-header">
        ...
        <div>
          ...
          <div class="feedback-card-header-sub">
            <?php echo $hasFeedback ? 'Submitted by ' . htmlspecialchars($feedback['student_name'] ?? 'student') : 'Awaiting staff feedback'; ?>
          </div>
        </div>
        <?php if ($hasFeedback): ?>
        ...
        <?php else: ?>
          <div class="fb-no-feedback-wrap">
            ...
            <div>
              ...
              <div class="fb-no-feedback-sub">No feedback has been submitted for this ticket.</div>
            </div>
          </div>
        <?php endif; ?>
      </div>
...
<div id="priModalBackdrop" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(10,20,50,.45);backdrop-filter:blur(4px);align-items:center;justify-content:center;">
  <div id="priModal" style="background:white;border-radius:14px;padding:32px 28px 24px;max-width:380px;width:90%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,.16);position:relative;animation:priModalIn .28s cubic-bezier(.34,1.56,.64,1);">
    ...
    <div style="background:#F9FAFB;border-radius:9px;padding:3px 0;margin-bottom:20px;border:1px solid #E5E7EB;">
      <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 16px;font-size:12.5px;"><span style="color:#9CA3AF;">Ticket</span><span id="priModalTicketId" style="font-family:monospace;font-weight:700;color:#7C3AED;font-size:11px;background:#F5F3FF;padding:2px 9px;border-radius:5px;"></span></div>
    ...
    <button onclick="closePriModal()" style="width:100%;padding:11px;border-radius:9px;border:none;background:#7C3AED;color:white;font-family:'DM Sans',sans-serif;font-size:14px;font-weight:600;cursor:pointer;">Close</button>
    <div id="priModalProgressBar" style="position:absolute;bottom:0;left:0;right:0;height:4px;background:#EDE9FE;border-radius:0 0 14px 14px;overflow:hidden;"><div id="priModalProgressFill" style="height:100%;width:100%;background:#7C3AED;border-radius:0 0 14px 14px;"></div></div>
  </div>
</div>
...
var priChipHtml = {
  low:    '<span style="display:inline-block;font-size:12.5px;font-weight:600;padding:2px 10px;border-radius:20px;background:#EFF6FF;color:#3B82F6">Low</span>',
  medium: '<span style="display:inline-block;font-size:12.5px;font-weight:600;padding:2px 10px;border-radius:20px;background:#FFFBEB;color:#F59E0B">Medium</span>',
  high:   '<span style="display:inline-block;font-size:12.5px;font-weight:600;padding:2px 10px;border-radius:20px;background:#FEF2F2;color:#EF4444">High</span>'
};
var statChipHtml = {
...
function openConfirmModal(){
  var statusEl=document.getElementById('status');
  if(!statusEl)return;
  var sta=statusEl.value;
  var map={
    open:'<span style="display:inline-block;font-size:12.5px;font-weight:600;padding:2px 10px;border-radius:20px;background:#FEF3C7;color:#D97706">Open</span>',
    in_progress:'<span style="display:inline-block;font-size:12.5px;font-weight:600;padding:2px 10px;border-radius:20px;background:#DBEAFE;color:#1D4ED8">In Progress</span>',
    closed:'<span style="display:inline-block;font-size:12.5px;font-weight:600;padding:2px 10px;border-radius:20px;background:#D1FAE5;color:#059669">Closed</span>'
  };
  document.getElementById('modalStatVal').innerHTML=map[sta]||sta;
  var isReopening=(currentTicketStatus==='closed'&&sta!=='closed');
...
var ra  = document.getElementById('replyAttachment');
var afp = document.getElementById('attachFilePreview');
var afn = document.getElementById('attachFileName');
var ar  = document.getElementById('attachRemove');
var aiw = document.getElementById('attachImgPreviewWrap');
var aip = document.getElementById('attachImgPreview');
if (ra) {
  ra.addEventListener('change', function() {
    if (this.files.length > 0) {
      var f = this.files[0], ext = f.name.split('.').pop().toLowerCase();
      afn.textContent = f.name; afp.style.display = 'flex';
      if (['jpg','jpeg','png','gif','webp'].indexOf(ext) !== -1) {
        var reader = new FileReader();
        reader.onload = function(e) { aip.src = e.target.result; aiw.style.display = 'block'; };
        reader.readAsDataURL(f);
      } else { aiw.style.display = 'none'; aip.src = ''; }
    }
  });
...
(function() {
  ...
  if (perPageEl) { perPageEl.addEventListener('change', function() { perPage = parseInt(this.value, 10); showPage(1); }); }
  showPage(1);
})();
</script>
</body>
