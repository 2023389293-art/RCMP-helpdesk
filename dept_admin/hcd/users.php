...
$ins  = $conn->prepare(
    ...
    foreach ($categories as $cat) {
        ...
        $catIdStmt = $conn->prepare("
            ...
            WHERE dept_id = 5 AND category_name LIKE CONCAT('%/ ', ?)
            LIMIT 1
        ");
...
$u = $conn->prepare("UPDATE staff SET full_name = ?, email = ?, phone = ?, category = ? WHERE staff_id = ? AND dept_id = 5");
...
if ($u->execute()) {
    ...
    $delSc = $conn->prepare("
        ...
        JOIN categories c ON c.category_id = sc.category_id
        WHERE sc.staff_id = ? AND c.dept_id = 5
    ");
...
$catStmt = $conn->prepare("SELECT category_id, category_name FROM categories WHERE dept_id = 5 ORDER BY category_name ASC");
$catStmt->execute();
