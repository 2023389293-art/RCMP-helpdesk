<?php
// dept_admin/hcd/ticket_detail.php 
require '_layout.php';
...
$adminStaffName = $_SESSION['staff_name'] ?? 'Admin HCD';
...
if ($ticketId !== '') {
    $stmt = $conn->prepare("
        ...
        WHERE c.ticket_id = ? AND c.dept_id = 5
        LIMIT 1
    ");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $ticket) {
    ...
    if ($action === 'reassign') {
        ...
        if ($newStaffId > 0) {
            ...
            manualAssignTicket($conn, 5, $ticketId, $newStaffId);
    ...
    if ($action === 'priority_only') {
        ...
        if (in_array($newPriority, $allowedPri)) {
            $freshStmt = $conn->prepare("SELECT priority, status FROM complaints WHERE ticket_id = ? AND dept_id = 5 LIMIT 1");
            ...
            if ($oldStatus !== 'closed' && $oldPriority !== $newPriority) {
                $upd = $conn->prepare("UPDATE complaints SET priority=?,updated_at=NOW() WHERE ticket_id=? AND dept_id=5");
...
if ($logStmt2) {
    ...
    in_array(strtolower($assignedNow['role'] ?? ''), ['admin', 'hod'])
        ...
        $freshStmt = $conn->prepare("SELECT priority, status, first_response_at FROM complaints WHERE ticket_id = ? AND dept_id = 5 LIMIT 1");
        $freshStmt->bind_param("s", $ticketId); $freshStmt->execute();
        $freshRow = $freshStmt->get_result()->fetch_assoc(); $freshStmt->close();
        $oldPriority      = $freshRow['priority']          ?? 'medium';
        $oldStatus        = $freshRow['status']            ?? 'open';
        ...
        $slaCheck = $conn->prepare("SELECT sla_start_at FROM complaints WHERE ticket_id=? AND dept_id=5 LIMIT 1");
        ...
        if (empty($slaRow['sla_start_at'])) {
            $slaSet = $conn->prepare("UPDATE complaints SET sla_start_at=? WHERE ticket_id=? AND dept_id=5");
        ...
        if (empty($oldFirstResponse) && in_array($newStatus, ['in_progress', 'closed'])) {
            $frSet = $conn->prepare("UPDATE complaints SET first_response_at=? WHERE ticket_id=? AND dept_id=5");
        ...
        if ($newStatus === 'closed' && $oldStatus !== 'closed') {
            $upd = $conn->prepare("UPDATE complaints SET priority=?,status=?,resolved_at=?,updated_at=NOW() WHERE ticket_id=? AND dept_id=5");
            $upd->bind_param("ssss", $newPriority, $newStatus, $nowMysql, $ticketId);
        } else {
            $upd = $conn->prepare("UPDATE complaints SET priority=?,status=?,updated_at=NOW() WHERE ticket_id=? AND dept_id=5");
...
$dsStmt = $conn->prepare("
    ...
    FROM staff
    WHERE dept_id = 5 AND status = 'active' AND role IN ('staff','admin')
...
<main class="main-content">
  ...
  <div class="not-found">
    ...
    <p>The ticket <strong><?= htmlspecialchars($ticketId) ?></strong> does not exist or does not belong to the Human Capital Department.</p>
          ...
          <div class="td-card-body">
            ...
            <div class="assigned-pill">
              ...
              <div style="flex:1;min-width:0;">
                ...
                <div class="assigned-role-tag"><?= $assignedStaff['role'] === 'admin' ? 'HCD Admin' : 'HCD Staff' ?></div>
              </div>
            </div>
            <?php else: ?>
