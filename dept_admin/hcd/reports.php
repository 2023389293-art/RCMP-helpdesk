...
$summary = $conn->query("
    ...
    FROM complaints c WHERE dept_id = 5 $dateWhere
")->fetch_assoc();
...
if ($days) {
    ...
    $prevTotal = $conn->query("SELECT COUNT(*) AS cnt FROM complaints c WHERE dept_id=5 $prevWhere")->fetch_assoc()['cnt'] ?? 0;
}
...
$topIssues = $conn->query("
    ...
    WHERE c.dept_id=5 $catDateWhere
...
$feedbackStats = $conn->query("
    ...
    WHERE c.dept_id=5 ".($days ? "AND f.created_at >= DATE_SUB(NOW(), INTERVAL {$days} DAY)" : '')
)->fetch_assoc();
...
<head>
  ...
  <title>HCD Admin — Reports | UniKL Help Desk</title>
...
<main class="main-content">
  ...
  <div class="page-header">
    <div>
      <div class="page-eyebrow">HCD Department</div>
