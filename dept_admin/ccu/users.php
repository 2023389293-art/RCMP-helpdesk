...
if (!$code || !$name || !$email || !$rawPw || empty($categories)) {
    ...
    $error = 'Phone number must be 10 or 11 digits (numbers only).';
        ...
        if ($chk->get_result()->num_rows > 0) {
            ...
            $dept = 'Corporate Communication Unit';
            $ins  = $conn->prepare(
    ...
    foreach ($categories as $cat) {
        ...
        $catIdStmt = $conn->prepare("
            ...
            WHERE dept_id = 3 AND category_name LIKE CONCAT('%/ ', ?)
            LIMIT 1
        ");
    ...
    if ($sid === (int)$_SESSION['staff_id']) {
        ...
        $u = $conn->prepare("UPDATE staff SET status = ? WHERE staff_id = ? AND dept_id = 3");
...
$u = $conn->prepare("UPDATE staff SET full_name = ?, email = ?, phone = ?, category = ? WHERE staff_id = ? AND dept_id = 3");
...
if ($u->execute()) {
    ...
    $delSc = $conn->prepare("
        ...
        JOIN categories c ON c.category_id = sc.category_id
        WHERE sc.staff_id = ? AND c.dept_id = 3
    ");
...
if (strlen($newPw) < 8 || strlen($newPw) > 10) {
    $error = 'Password must be between 8 and 10 characters.';
            ...
            $u = $conn->prepare("UPDATE staff SET password_hash = ? WHERE staff_id = ? AND dept_id = 3");
    ...
    if ($action === 'delete') {
        ...
        if ($sid === (int)$_SESSION['staff_id']) {
            ...
            if ($ticketCount > 0) {
                ...
                $d = $conn->prepare("DELETE FROM staff WHERE staff_id = ? AND dept_id = 3");
                $d->bind_param("i", $sid);
                if ($d->execute()) {
...
$catStmt = $conn->prepare("SELECT category_id, category_name FROM categories WHERE dept_id = 3 ORDER BY category_name ASC");
$catStmt->execute();
...
<head>
  ...
  <title>CCU Admin — Manage Users | UniKL Help Desk</title>
