...
$cats = $conn->query("SELECT category_id, category_name FROM categories WHERE dept_id = 3 ORDER BY category_name")->fetch_all(MYSQLI_ASSOC);

// ── KPI STATUS COUNTS (always unfiltered, for the cards) ──
$kpiCounts = ['open'=>0,'in_progress'=>0,'closed'=>0];
$kpiRes = $conn->query("SELECT status, COUNT(*) AS n FROM complaints WHERE dept_id = 3 GROUP BY status");
...
<head>
  ...
  <title>CCU Admin — All Tickets | UniKL Help Desk</title>
