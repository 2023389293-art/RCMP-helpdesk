...
// ── Stats ──────────────────────────────────────────────────────────────────
$stats = [];
$r = $conn->query("SELECT COUNT(*) AS n FROM complaints WHERE dept_id = 3");
$stats['total'] = $r->fetch_assoc()['n'];
$r = $conn->query("SELECT COUNT(*) AS n FROM complaints WHERE dept_id = 3 AND status = 'open'");
$stats['open'] = $r->fetch_assoc()['n'];
$r = $conn->query("SELECT COUNT(*) AS n FROM complaints WHERE dept_id = 3 AND status = 'in_progress'");
$stats['in_progress'] = $r->fetch_assoc()['n'];
$r = $conn->query("SELECT COUNT(*) AS n FROM complaints WHERE dept_id = 3 AND status = 'closed'");
$stats['closed'] = $r->fetch_assoc()['n'];
$r = $conn->query("SELECT COUNT(*) AS n FROM staff WHERE dept_id = 3 AND status = 'active' AND role = 'staff'");
$stats['staff'] = $r->fetch_assoc()['n'];
$r = $conn->query("SELECT ROUND(AVG(f.rating),1) AS avg_r, COUNT(*) AS total_r FROM ticket_feedback f JOIN complaints c ON c.ticket_id=f.ticket_id WHERE c.dept_id=3");
...
$ratingBreakdown = [];
for ($i = 5; $i >= 1; $i--) {
    $r = $conn->query("SELECT COUNT(*) AS n FROM ticket_feedback f JOIN complaints c ON c.ticket_id=f.ticket_id WHERE c.dept_id=3 AND f.rating=$i");
    $ratingBreakdown[$i] = (int)$r->fetch_assoc()['n'];
}

// Top departments
$topDepts = $conn->query("SELECT c.my_department AS dept_name, COUNT(*) AS n FROM complaints c WHERE c.dept_id=3 GROUP BY c.my_department ORDER BY n DESC LIMIT 5")->fetch_all(MYSQLI_ASSOC);
$totalComplaints = max($stats['total'], 1);

// Category breakdown
$catBreakdown = $conn->query("SELECT cat.category_name, COUNT(*) AS n FROM complaints c JOIN categories cat ON cat.category_id=c.category_id WHERE c.dept_id=3 GROUP BY c.category_id ORDER BY n DESC")->fetch_all(MYSQLI_ASSOC);
...
// Calendar tickets
$calTickets = $conn->query("SELECT c.ticket_id, c.title, c.status, c.priority, DATE_FORMAT(c.created_at,'%Y-%m-%d') AS ticket_date, cat.category_name FROM complaints c JOIN categories cat ON cat.category_id=c.category_id WHERE c.dept_id=3 ORDER BY c.created_at DESC")->fetch_all(MYSQLI_ASSOC);
...
<head>
  ...
  <title>CCU Admin — Dashboard | UniKL Help Desk</title>
  ...
  <style>
    ...
    *{box-sizing:border-box;}
    ...
    /* MIDDLE ROW */
    .middle-row{display:grid;grid-template-columns:310px 1fr;gap:16px;margin-bottom:20px;align-items:stretch;}
    ...
    .cal-header{display:flex;flex-direction:column;align-items:center;gap:10px;margin-bottom:14px;}
    ...
    /* CHART PANEL */
    .cal-chart-panel{background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:20px;display:flex;flex-direction:column;height:100%;}
    #chartArea{flex:1;display:flex;flex-direction:column;min-height:0;}
    #chartArea > div{position:relative;height:320px;}
    ...
    .chart-empty{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;color:var(--text-muted);gap:10px;text-align:center;}
    ...
    .sla-assignee{display:inline-flex;align-items:center;gap:5px;margin-top:5px;background:var(--indigo-50);border:1px solid var(--indigo-200);border-radius:20px;padding:2px 8px;font-size:10px;font-weight:600;color:var(--indigo-600);max-width:180px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
    .sla-assignee svg{width:11px;height:11px;fill:none;stroke:var(--indigo-500);stroke-width:2;flex-shrink:0;}
    .sla-unassigned{display:inline-flex;align-items:center;gap:5px;margin-top:5px;background:#fafafa;border:1px dashed var(--navy-300);border-radius:20px;padding:2px 8px;font-size:10px;font-weight:600;color:var(--text-muted);}
    .sla-unassigned svg{width:11px;height:11px;fill:none;stroke:var(--navy-300);stroke-width:2;flex-shrink:0;}
    ...
    .rating-faces{display:flex;justify-content:center;gap:6px;margin:10px 0 6px;}
    .rating-face{width:28px;height:28px;opacity:.25;transition:opacity .15s;}
    .rating-face.active{opacity:1;}
    .rb-face-icon{width:20px;height:20px;flex-shrink:0;}
  </style>
</head>
<body>
...
<main class="main-content">
  ...
  <div class="dash-header">
    <div>
      <div class="dash-eyebrow">CCU Help Desk · Admin Panel</div>
      <h1 class="dash-h1">CCU <span class="accent">Dashboard</span></h1>
    </div>
...
function getTicketsForPrefix(prefix) { return ALL_TICKETS.filter(t=>t.ticket_date.startsWith(prefix)); }
