<?php
// dept_admin/it/_layout.php
// Shared layout helpers for IT Admin pages

if (session_status() === PHP_SESSION_NONE) session_start();

// Auth guard
if (empty($_SESSION['staff_id']) || $_SESSION['dept_folder'] !== 'it' || $_SESSION['staff_role'] !== 'admin') {
    header("Location: ../../staff_login.php");
    exit;
}

require_once '../../db_connect.php';

$adminName   = $_SESSION['staff_name']  ?? 'Admin';
$adminEmail  = $_SESSION['staff_email'] ?? '';
$currentPage = basename($_SERVER['PHP_SELF'], '.php'); // dashboard | tickets | users | reports

function nav_item($href, $icon, $label, $id, $current) {
    $active = ($id === $current);
    $cls    = $active ? 'nav-item active' : 'nav-item';
    $pip    = $active ? '<span class="nav-pip"></span>' : '';

    return '<a href="' . htmlspecialchars($href) . '" class="' . $cls . '">'
         . '<span class="nav-icon">' . $icon . '</span>'
         . '<span class="nav-label">' . $label . '</span>'
         . $pip
         . '</a>';
}
?>