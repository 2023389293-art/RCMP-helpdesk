...
if (!$code || !$name || !$email || !$rawPw || empty($categories)) {
    ...
    if ($sid === (int)$_SESSION['staff_id']) {
        ...
        $u = $conn->prepare("UPDATE staff SET status = ? WHERE staff_id = ? AND dept_id = 4");
...
$u = $conn->prepare("UPDATE staff SET staff_code = ?, full_name = ?, email = ?, phone = ?, category = ? WHERE staff_id = ? AND dept_id = 4");
...
if (strlen($newPw) < 8 || strlen($newPw) > 10) {
    $error = 'Password must be between 8 and 10 characters.';
            ...
            $u = $conn->prepare("UPDATE staff SET password_hash = ? WHERE staff_id = ? AND dept_id = 4");
    ...
    if ($action === 'delete') {
        ...
        if ($sid === (int)$_SESSION['staff_id']) {
            ...
            if ($ticketCount > 0) {
                ...
                $d = $conn->prepare("DELETE FROM staff WHERE staff_id = ? AND dept_id = 4");
                $d->bind_param("i", $sid);
                if ($d->execute()) {
...
$catStmt = $conn->prepare("SELECT category_id, category_name FROM categories WHERE dept_id = 4 ORDER BY category_name ASC");
$catStmt->execute();
...
<head>
  ...
  <title>IT Admin — Manage Users | UniKL Help Desk</title>
