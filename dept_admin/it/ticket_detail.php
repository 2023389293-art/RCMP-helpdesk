...
$adminStaffName = $_SESSION['staff_name'] ?? 'Admin IT';
...
if ($ticketId !== '') {
    $stmt = $conn->prepare("
        ...
        WHERE c.ticket_id = ? AND c.dept_id = 4
        LIMIT 1
    ");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $ticket) {
    ...
    if ($action === 'reassign') {
        ...
        if ($newStaffId > 0) {
            ...
            $clrVendor = $conn->prepare("UPDATE complaints SET assigned_vendor_id=NULL, assigned_vendor_name=NULL WHERE ticket_id=? AND dept_id=4");
            $clrVendor->bind_param("s", $ticketId); $clrVendor->execute(); $clrVendor->close();

            manualAssignTicket($conn, 4, $ticketId, $newStaffId);
...
HTML;
    ...
    if ($action === 'priority_only') {
        ...
        if (in_array($newPriority, $allowedPri)) {
            $freshStmt = $conn->prepare("SELECT priority, status FROM complaints WHERE ticket_id = ? AND dept_id = 4 LIMIT 1");
            ...
            if ($oldStatus !== 'closed' && $oldPriority !== $newPriority) {
                $upd = $conn->prepare("UPDATE complaints SET priority=?,updated_at=NOW() WHERE ticket_id=? AND dept_id=4");
...
if ($logStmt2) {
    ...
    in_array(strtolower($assignedNow['role'] ?? ''), ['admin', 'hod'])
        ...
        $freshStmt = $conn->prepare("SELECT priority, status, first_response_at FROM complaints WHERE ticket_id = ? AND dept_id = 4 LIMIT 1");
        $freshStmt->bind_param("s", $ticketId); $freshStmt->execute();
        $freshRow = $freshStmt->get_result()->fetch_assoc(); $freshStmt->close();
        $oldPriority      = $freshRow['priority']          ?? 'medium';
        $oldStatus        = $freshRow['status']            ?? 'open';
        ...
        $slaCheck = $conn->prepare("SELECT sla_start_at FROM complaints WHERE ticket_id=? AND dept_id=4 LIMIT 1");
        ...
        if (empty($slaRow['sla_start_at'])) {
            $slaSet = $conn->prepare("UPDATE complaints SET sla_start_at=? WHERE ticket_id=? AND dept_id=4");
        ...
        if (empty($oldFirstResponse) && in_array($newStatus, ['in_progress', 'closed'])) {
            $frSet = $conn->prepare("UPDATE complaints SET first_response_at=? WHERE ticket_id=? AND dept_id=4");
        ...
        if ($newStatus === 'closed' && $oldStatus !== 'closed') {
            $upd = $conn->prepare("UPDATE complaints SET priority=?,status=?,resolved_at=?,updated_at=NOW() WHERE ticket_id=? AND dept_id=4");
            $upd->bind_param("ssss", $newPriority, $newStatus, $nowMysql, $ticketId);
        } else {
            $upd = $conn->prepare("UPDATE complaints SET priority=?,status=?,updated_at=NOW() WHERE ticket_id=? AND dept_id=4");
...
$dsStmt = $conn->prepare("
    ...
    FROM staff
    WHERE dept_id = 4 AND status = 'active' AND role IN ('staff','admin')
...
// ── Fetch active vendors linked to dept 4 ────────────────────────────────────
$deptVendorList = [];
$dvStmt = $conn->prepare("
    SELECT v.vendor_id, v.company_name,
           vs.full_name AS pic_name
    ...
    LEFT JOIN vendor_staff vs ON vs.vendor_id = v.vendor_id AND vs.is_primary = 1
    WHERE vd.dept_id = 4 AND v.status = 'active'
...
?>
<title><?= htmlspecialchars($catTitle) ?> | UniKL Help Desk – IT Admin</title>
...
<main class="main-content">
  ...
  $catDisplay = preg_replace('/^[^\/]+\/\s*/', '', $rawCat); // strips "IT Dept / " prefix
?>
...
<div class="assigned-role-tag" style="color:#7C3AED;">🏢 Vendor</div>
            ...
            <div class="assigned-pill">
              ...
              <div style="flex:1;min-width:0;">
                ...
                <div class="assigned-role-tag"><?= $assignedStaff['role'] === 'admin' ? 'IT Admin' : 'IT Staff' ?></div>
              </div>
            </div>
            <?php else: ?>
