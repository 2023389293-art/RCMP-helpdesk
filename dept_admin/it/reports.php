...
$summary = $conn->query("
    ...
    FROM complaints c WHERE dept_id = 4 $dateWhere
")->fetch_assoc();
...
if ($days) {
    ...
    $prevTotal = $conn->query("SELECT COUNT(*) AS cnt FROM complaints c WHERE dept_id=4 $prevWhere")->fetch_assoc()['cnt'] ?? 0;
}
...
$topIssues = $conn->query("
    ...
    WHERE c.dept_id=4 $catDateWhere
...
$feedbackStats = $conn->query("
    ...
    WHERE c.dept_id=4 ".($days ? "AND f.created_at >= DATE_SUB(NOW(), INTERVAL {$days} DAY)" : '')
)->fetch_assoc();
