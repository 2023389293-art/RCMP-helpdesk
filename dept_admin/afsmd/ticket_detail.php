...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $ticket) {
    ...
    if ($action === 'reassign') {
        ...
        if ($newStaffId > 0) {
            ...
            $clrVendor = $conn->prepare("UPDATE complaints SET assigned_vendor_id=NULL, assigned_vendor_name=NULL WHERE ticket_id=? AND dept_id=1");
...
HTML;
            ...
            if ($vRow) {
                ...
                $updVendor = $conn->prepare("UPDATE complaints SET assigned_to=NULL, assigned_vendor_id=?, assigned_vendor_name=?, updated_at=NOW() WHERE ticket_id=? AND dept_id=1");
...
HTML;
            ...
            if ($newStatus === 'closed' && $oldStatus !== 'closed') {
                $wipe = $conn->prepare("UPDATE complaints SET submitter_name = NULL, phone = NULL WHERE ticket_id = ? AND dept_id = 1");
                if ($wipe) {
...
// ── Fetch active vendors linked to dept 1 ────────────────────────────────────
$deptVendorList = [];
$dvStmt = $conn->prepare("
    SELECT v.vendor_id, v.company_name,
           vs.full_name AS pic_name
    ...
    LEFT JOIN vendor_staff vs ON vs.vendor_id = v.vendor_id AND vs.is_primary = 1
    WHERE vd.dept_id = 1 AND v.status = 'active'
...
// Move $isAdminAssigned here so $isClosed is already defined
// Admin can update if: ticket assigned to them OR assigned to any admin/hod role
          ...
          <div class="td-card-body">
            <div class="ti-meta-grid">
              <div>
                ...
                <div class="ti-meta-value"><?= htmlspecialchars(preg_replace('/^AFSMD\s*\/\s*/i', '', $ticket['category_name'] ?? '—')) ?></div>
              </div>
              <div>
