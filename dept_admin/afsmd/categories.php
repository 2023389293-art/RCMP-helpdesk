<?php
// dept_admin/afsmd/categories.php 
...
$stmt = $conn->prepare(
    "SELECT SUM(status='open') AS oc, SUM(status='closed') AS cc
     FROM complaints WHERE dept_id = 1"
);
$stmt->execute();
...
// ── Fetch AFSMD categories (dept_id = 1) ─────────────────────────────────────
$categories = [];
$deptId = 1;
$stmt = $conn->prepare(
      ...
      <div class="tbl-card">
        <div class="tbl-header">
          <div class="tbl-header-left">
            ...
            <div>
              <div style="font-size:14px;font-weight:600;color:var(--gray-900);">AFSMD Categories</div>
              <div style="font-size:12px;color:var(--gray-500);margin-top:1px;">Manage AFSMD complaint categories</div>
            </div>
          ...
          <table>
            ...
            <tbody id="cat-tbody">
              ...
              <tr data-name="<?= strtolower(htmlspecialchars($cat['category_name'])) ?>">
                ...
                <td>
                  <div class="cat-name"><?= htmlspecialchars(preg_replace('/^AFSMD\s*\/\s*/i', '', $cat['category_name'])) ?></div>
                </td>
                <td>
