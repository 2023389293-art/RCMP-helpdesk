...
$summary = $conn->query("
    ...
    FROM complaints c WHERE dept_id = 1 $dateWhere
")->fetch_assoc();
...
if ($days) {
    ...
    $prevTotal = $conn->query("SELECT COUNT(*) AS cnt FROM complaints c WHERE dept_id=1 $prevWhere")->fetch_assoc()['cnt'] ?? 0;
}
...
// ── Resolve submitter name/email based on type ────────────────────────
    ...
    // ── Always hide submitter name — email only ────────────────────────
...
$topIssues = $conn->query("
    ...
    WHERE c.dept_id=1 $catDateWhere
...
$feedbackStats = $conn->query("
    ...
    WHERE c.dept_id=1 ".($days ? "AND f.created_at >= DATE_SUB(NOW(), INTERVAL {$days} DAY)" : '')
)->fetch_assoc();
...
<head>
  ...
  <title>AFSMD Admin — Reports | UniKL Help Desk</title>
        ...
        <table class="data-table" id="staffTable">
          ...
          <tbody id="staffTableBody">
            ...
            <?php else: foreach ($staffActivity as $i => $s):
              ...
              // Format avg respond time nicely
              if ($avgRespondH === null) {
                  $avgRespondFmt = '—';
...
<script src="js/staff-reportt.js"></script>
<?php endif; ?>
