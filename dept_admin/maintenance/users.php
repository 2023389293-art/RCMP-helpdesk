...
$category   = !empty($categories) ? $categories[0] : '';
    ...
    foreach ($categories as $cat) {
        ...
        $catIdStmt = $conn->prepare("
            ...
            WHERE dept_id = 2 AND category_name LIKE CONCAT('%/ ', ?)
            LIMIT 1
        ");
    ...
    if ($sid === (int)$_SESSION['staff_id']) {
        ...
        $u = $conn->prepare("UPDATE staff SET status = ? WHERE staff_id = ? AND dept_id = 2");
...
$u = $conn->prepare("UPDATE staff SET staff_code = ?, full_name = ?, email = ?, phone = ?, category = ? WHERE staff_id = ? AND dept_id = 2");
...
if ($u->execute()) {
    ...
    $delSc = $conn->prepare("
        ...
        JOIN categories c ON c.category_id = sc.category_id
        WHERE sc.staff_id = ? AND c.dept_id = 2
    ");
    ...
    if ($action === 'reset_password') {
        ...
        if (strlen($newPw) < 8 || strlen($newPw) > 10) {
            ...
            $u = $conn->prepare("UPDATE staff SET password_hash = ? WHERE staff_id = ? AND dept_id = 2");
    ...
    if ($action === 'delete') {
        ...
        if ($sid === (int)$_SESSION['staff_id']) {
            ...
            if ($ticketCount > 0) {
                ...
                $d = $conn->prepare("DELETE FROM staff WHERE staff_id = ? AND dept_id = 2");
                $d->bind_param("i", $sid);
                if ($d->execute()) {
...
// Load categories for Maintenance dept from database
$catStmt = $conn->prepare("SELECT category_id, category_name FROM categories WHERE dept_id = 2 ORDER BY category_name ASC");
$catStmt->execute();
...
<head>
  ...
  <title>Maintenance Admin — Manage Users | UniKL Help Desk</title>
...
// Auto-clear Add modal fields on open
