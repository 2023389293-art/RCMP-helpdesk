...
$successMessages = [
    ...
    'password_reset' => 'Vendor password reset successfully.',
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    SELECT v.company_name, v.email,
           vs.full_name AS pic_name, vs.email AS pic_email
    FROM vendors v
...
$vInfo->close();
if ($vRow) {
    // Email the primary staff contact if available, otherwise the company email
    $toEmail = !empty($vRow['pic_email']) ? $vRow['pic_email'] : $vRow['email'];
...
$rInfo->close();
        ...
        try {
            ...
            if ($rVendorRow) {
    $toEmail = !empty($rVendorRow['pic_email']) ? $rVendorRow['pic_email'] : $rVendorRow['email'];
   ...
   if ($action === 'activate' && $vendor_id) {
    ...
    // RESET PASSWORD
    if ($action === 'reset_password' && $vendor_id) {
        $newPw = trim($_POST['new_password'] ?? '');
        if (strlen($newPw) < 8 || strlen($newPw) > 10) {
            $error = 'Password must be between 8 and 10 characters.';
        } else {
            $hash = password_hash($newPw, PASSWORD_BCRYPT);
            $s = $conn->prepare("UPDATE vendors SET password_hash = ? WHERE vendor_id = ?");
            $s->bind_param("si", $hash, $vendor_id);
            if ($s->execute()) {
                header('Location: vendors.php?success=password_reset');
                exit;
            }
...
$sql = "
    SELECT v.vendor_id, v.company_name, v.address, v.city, v.state, v.postcode,
           ...
           vs.full_name AS pic_name, vs.email AS pic_email, vs.phone AS pic_phone,
           vs.position AS pic_position
    FROM vendors v
...
<td><?= htmlspecialchars($v['phone'] ?? '—') ?></td>
  ...
  '<?= date('d M Y, g:i A', strtotime($v['created_at'])) ?>'
              ...
              <button type="button" class="icon-btn btn-reset" title="Reset Password"
                      onclick="openResetModal(<?= $v['vendor_id'] ?>, '<?= htmlspecialchars(addslashes($v['company_name'])) ?>')">
    ...
    <form method="POST" class="modal-form" id="addVendorForm" onsubmit="return validateAddVendorForm()" autocomplete="off">
      ...
      <div class="field">
        ...
        <div style="position:relative;">
          <input type="password" name="password" id="addVendorPwInput"
                 placeholder="8–10 characters" required minlength="8" maxlength="10"
                 autocomplete="new-password" style="width:100%; padding-right:42px;"/>
          <button type="button" id="addVendorPwToggle"
            ...
            <svg id="addVendorEyeIcon" viewBox="0 0 24 24" width="16" height="16" fill="none"
        ...
        <small style="color:#9ca3af;font-size:0.75rem;">Must be between 8 and 10 characters.</small>
      </div>
...
<div class="modal-overlay" id="resetModal">
  <div class="modal" style="max-width:420px;">
    ...
    <form method="POST" class="modal-form" id="resetForm">
      <input type="hidden" name="action"    value="reset_password"/>
      <input type="hidden" name="vendor_id" id="resetVendorId"/>
      <div class="field">
      ...
      <div class="modal-footer">
        ...
        <button type="submit" class="btn-primary-sm">Reset Password</button>
      </div>
    </form>
  </div>
</div>
...
<div class="modal-overlay" id="viewModal">
  ...
  <div style="border-top:1px solid #f1f5f9;padding-top:12px;">
    <div style="font-size:.72rem;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px;">Staff List</div>
    <div id="vStaffList"></div>
  </div>
...
function openResetModal(id, name) {
  document.getElementById('resetVendorId').value       = id;
  document.getElementById('resetName').textContent     = name;
  document.getElementById('resetPasswordInput').value  = '';
  document.getElementById('resetPasswordInput').type   = 'password';
  document.getElementById('resetEyeIcon').innerHTML    = eyeOpen;
  openModal('resetModal');
}

// Eye toggle for reset modal
document.getElementById('resetPwToggle').addEventListener('click', function () {
  ...
  document.getElementById('resetEyeIcon').innerHTML = isPw ? eyeShut : eyeOpen;
});
...
function openViewModal(vendorId, company, email, phone, address, status, created) {
  ...
  // Load staff list via AJAX
  const staffBox = document.getElementById('vStaffList');
  staffBox.innerHTML = '<p style="font-size:.82rem;color:#9ca3af;">Loading…</p>';
  fetch(`vendors_staff_ajax.php?vendor_id=${vendorId}`)
    .then(r => r.json())
    .then(data => {
      if (!data.length) {
        staffBox.innerHTML = '<p style="font-size:.82rem;color:#9ca3af;font-style:italic;">No staff added yet.</p>';
        return;
      }
      let html = `<table style="width:100%;border-collapse:collapse;font-size:.82rem;">
        <thead>
          <tr style="background:#f7f8fa;border-bottom:1px solid #e4e7ed;">
            <th style="padding:7px 10px;text-align:left;color:#6b7280;font-size:.68rem;text-transform:uppercase;letter-spacing:.05em;">Name</th>
            <th style="padding:7px 10px;text-align:left;color:#6b7280;font-size:.68rem;text-transform:uppercase;letter-spacing:.05em;">Position</th>
            <th style="padding:7px 10px;text-align:left;color:#6b7280;font-size:.68rem;text-transform:uppercase;letter-spacing:.05em;">Phone</th>
            <th style="padding:7px 10px;text-align:left;color:#6b7280;font-size:.68rem;text-transform:uppercase;letter-spacing:.05em;">Email</th>
          </tr>
        </thead><tbody>`;
      data.forEach(s => {
        html += `<tr style="border-bottom:1px solid #f1f5f9;">
          <td style="padding:8px 10px;font-weight:${s.is_primary?'700':'400'};color:#111827;">
            ${s.full_name}
            ${s.is_primary ? '<span style="margin-left:5px;font-size:.65rem;padding:1px 7px;background:#dbeafe;color:#1d4ed8;border-radius:10px;font-weight:600;">Primary</span>' : ''}
          </td>
          <td style="padding:8px 10px;color:#374151;">${s.position || '—'}</td>
          <td style="padding:8px 10px;color:#374151;">${s.phone || '—'}</td>
          <td style="padding:8px 10px;color:#374151;">${s.email || '—'}</td>
        </tr>`;
      });
      html += '</tbody></table>';
      staffBox.innerHTML = html;
    })
    .catch(() => {
      staffBox.innerHTML = '<p style="font-size:.82rem;color:#ef4444;">Failed to load staff.</p>';
    });
...
// ── Add Vendor Modal ─────────────────────────────────────────────────────
const addVendorPwInput = document.getElementById('addVendorPwInput');
const addVendorEyeIcon = document.getElementById('addVendorEyeIcon');

document.getElementById('addVendorPwToggle').addEventListener('click', function () {
  const isPw = addVendorPwInput.type === 'password';
  addVendorPwInput.type = isPw ? 'text' : 'password';
  addVendorEyeIcon.innerHTML = isPw ? eyeShut : eyeOpen;
});
...
window.openModal = function(id) {
  if (id === 'addVendorModal') {
    ...
    addVendorPwInput.type = 'password';
    addVendorEyeIcon.innerHTML = eyeOpen;
  }
  document.getElementById(id).classList.add('open');
};

function validateAddVendorForm() {
  const pw = addVendorPwInput.value;
  if (pw.length < 8 || pw.length > 10) {
    alert('Password must be between 8 and 10 characters.');
    addVendorPwInput.focus();
    return false;
  }
  return true;
}
