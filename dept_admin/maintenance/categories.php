<?php
// dept_admin/maintenance/categories.php
...
$stmt = $conn->prepare(
    "SELECT SUM(status='open') AS oc, SUM(status='closed') AS cc
     FROM complaints WHERE dept_id = 2"
);
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($action === 'edit') {
        ...
        $suffix = trim($_POST['category_name'] ?? '');
$suffix = trim(preg_replace('/^(Maintenance\s+Department\s*\/\s*)+/i', '', $suffix));
$name   = $suffix !== '' ? 'Maintenance Department / ' . $suffix : '';
        if (!$id || empty($suffix)) {
...
// ── Fetch Maintenance categories (dept_id = 2) ────────────────────────────────────────
$categories = [];
$deptId = 2;
$stmt = $conn->prepare(
...
<head>
  ...
  <title>Categories | UniKL Help Desk – Maintenance Admin</title>
      ...
      <div class="tbl-card">
        <div class="tbl-header">
          <div class="tbl-header-left">
            ...
            <div>
              <div style="font-size:14px;font-weight:600;color:var(--gray-900);">Maintenance Department Categories</div>
          ...
          <table>
            ...
            <tbody id="cat-tbody">
              ...
              <tr data-name="<?= strtolower(htmlspecialchars($cat['category_name'])) ?>">
                ...
                <td>
                  <div class="cat-name"><?= htmlspecialchars(preg_replace('/^Maintenance Department\s*\/\s*/i', '', $cat['category_name'])) ?></div>
                </td>
                <td>
...
<div class="prefix-wrap">
    <input type="text" id="category_name" name="category_name"
      ...
      placeholder="e.g. Air Conditioner &amp; Ventilation"
...
function startEdit(id, fullName) {
  var suffix = fullName.replace(/^(Maintenance\s+Department\s*\/\s*)+/i, '').trim();
