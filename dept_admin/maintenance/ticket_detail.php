...
$adminStaffName = $_SESSION['staff_name'] ?? 'Admin Maintenance';
...
if ($ticketId !== '') {
    $stmt = $conn->prepare("
        ...
        WHERE c.ticket_id = ? AND c.dept_id = 2
        LIMIT 1
    ");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $ticket) {
    ...
    if ($action === 'reassign') {
        ...
        if ($newStaffId > 0) {
            ...
            // Clear vendor assignment when reassigning to staff
            $clrVendor = $conn->prepare("UPDATE complaints SET assigned_vendor_id=NULL, assigned_vendor_name=NULL WHERE ticket_id=? AND dept_id=2");
            $clrVendor->bind_param("s", $ticketId); $clrVendor->execute(); $clrVendor->close();

            manualAssignTicket($conn, 2, $ticketId, $newStaffId);
...
HTML;
                    ...
                    error_log("[UniKL Mail] Staff assign notify failed for {$ticketId}: ".$e->getMessage());
                }
            }
            ...
            if ($vRow) {
                ...
                $updVendor = $conn->prepare("UPDATE complaints SET assigned_to=NULL, assigned_vendor_id=?, assigned_vendor_name=?, updated_at=NOW() WHERE ticket_id=? AND dept_id=2");
                $updVendor->bind_param("iss", $newVendorId, $vendorDisplayName, $ticketId);
                ...
                if ($asnLog) {
                    ...
                    $asnLog->bind_param("sissss", $ticketId, $adminStaffId, $adminStaffName, $oldName, $vendorDisplayName, $remarksFull);
                    $asnLog->execute(); $asnLog->close();
                }
...
HTML;
                        ...
                        error_log("[UniKL Mail] Vendor assign notify failed for {$ticketId}: ".$e->getMessage());
                    }
                }

                $_SESSION['flash_success'] = 'Ticket assigned to vendor <strong>' . htmlspecialchars($vRow['company_name']) . '</strong>.';
            } else {
    ...
    if ($action === 'priority_only') {
        ...
        if (in_array($newPriority, $allowedPri)) {
            $freshStmt = $conn->prepare("SELECT priority, status FROM complaints WHERE ticket_id = ? AND dept_id = 2 LIMIT 1");
            ...
            if ($oldStatus !== 'closed' && $oldPriority !== $newPriority) {
                $upd = $conn->prepare("UPDATE complaints SET priority=?,updated_at=NOW() WHERE ticket_id=? AND dept_id=2");
...
if ($logStmt2) {
    ...
    if ($action === 'admin_update') {
        ...
        $freshStmt = $conn->prepare("SELECT priority, status, first_response_at FROM complaints WHERE ticket_id = ? AND dept_id = 2 LIMIT 1");
        $freshStmt->bind_param("s", $ticketId); $freshStmt->execute();
        $freshRow = $freshStmt->get_result()->fetch_assoc(); $freshStmt->close();
        $oldPriority      = $freshRow['priority']          ?? 'medium';
        $oldStatus        = $freshRow['status']            ?? 'open';
        ...
        $slaCheck = $conn->prepare("SELECT sla_start_at FROM complaints WHERE ticket_id=? AND dept_id=2 LIMIT 1");
        ...
        if (empty($slaRow['sla_start_at'])) {
            $slaSet = $conn->prepare("UPDATE complaints SET sla_start_at=? WHERE ticket_id=? AND dept_id=2");
        ...
        if (empty($oldFirstResponse) && in_array($newStatus, ['in_progress', 'closed'])) {
            $frSet = $conn->prepare("UPDATE complaints SET first_response_at=? WHERE ticket_id=? AND dept_id=2");
        ...
        if ($newStatus === 'closed' && $oldStatus !== 'closed') {
            $upd = $conn->prepare("UPDATE complaints SET priority=?,status=?,resolved_at=?,updated_at=NOW() WHERE ticket_id=? AND dept_id=2");
            $upd->bind_param("ssss", $newPriority, $newStatus, $nowMysql, $ticketId);
        } else {
            $upd = $conn->prepare("UPDATE complaints SET priority=?,status=?,updated_at=NOW() WHERE ticket_id=? AND dept_id=2");
...
if ($ticket) {
    ...
    // Also check for vendor assignment
...
// ── Fetch dept staff list for reassign dropdown (staff + admins of dept 2) ───
// We show both staff and admin roles so admin can assign to themselves
$deptStaffList = [];
$dsStmt = $conn->prepare("
    SELECT staff_id, full_name, role
    FROM staff
    WHERE dept_id = 2 AND status = 'active' AND role IN ('staff','admin')
...
// ── Fetch active vendors linked to dept 2 ────────────────────────────────────
$deptVendorList = [];
$dvStmt = $conn->prepare("
    SELECT v.vendor_id, v.company_name,
           vs.full_name AS pic_name
    ...
    LEFT JOIN vendor_staff vs ON vs.vendor_id = v.vendor_id AND vs.is_primary = 1
    WHERE vd.dept_id = 2 AND v.status = 'active'
...
// Admin can update if assigned to ticket OR if any admin/hod role is assigned OR ticket is vendor-assigned
$isAdminAssigned = (!$isClosed && (
    ($assignedStaff && (
...
<main class="main-content">
  ...
  <div class="not-found">
    ...
    <p>The ticket <strong><?= htmlspecialchars($ticketId) ?></strong> does not exist or does not belong to the Maintenance Department.</p>
          ...
          <div class="td-card-body">
            ...
            <div class="assigned-pill" style="border-left: 3px solid #7C3AED;">
              <div class="assigned-avatar" style="background:#F3E8FF;color:#7C3AED;">
                <?= strtoupper(substr($assignedVendor['company_name'], 0, 1)) ?>
              </div>
              <div style="flex:1;min-width:0;">
                <div class="assigned-name"><?= htmlspecialchars($assignedVendor['company_name']) ?></div>
<div class="assigned-role-tag" style="color:#7C3AED;">🏢 Vendor</div>
              </div>
            </div>
            ...
            <div class="assigned-pill">
              ...
              <div style="flex:1;min-width:0;">
                ...
                <div class="assigned-role-tag"><?= $assignedStaff['role'] === 'admin' ? 'Maintenance Admin' : 'Maintenance Staff' ?></div>
              </div>
            </div>
            <?php else: ?>
            ...
            <?php if (!empty($deptStaffList) || !empty($deptVendorList)): ?>
  ...
  <div class="reassign-row">
    <select class="reassign-select" id="reassignSelect" onchange="handleReassignChange(this)">
      <option value=""><?= $curStat === 'in_progress' ? '— Select staff or vendor —' : '— Select staff —' ?></option>
      ...
      <optgroup label="─── Vendors ───">
        ...
        <option value="vendor_<?= $v['vendor_id'] ?>">
            ...
            <div class="tl-changes" style="margin-top:5px">
              ...
              <div class="tl-row" style="margin-top:4px;">
                ...
                <span class="tl-chip-name" style="background:#F3E8FF;color:#7C3AED;">
                  👤 <?= htmlspecialchars($log['vendor_staff_name']) ?>
                  <?php if (!empty($log['changed_by']) && $log['changed_by'] !== $log['vendor_staff_name']): ?>
                  <span style="color:#9CA3AF;font-weight:400;"> · <?= htmlspecialchars($log['changed_by']) ?></span>
                  <?php endif; ?>
                </span>
              </div>
              <?php endif; ?>
...
function handleReassignChange(sel) {
  ...
  // Split "staff_123" or "vendor_456" into the hidden inputs
