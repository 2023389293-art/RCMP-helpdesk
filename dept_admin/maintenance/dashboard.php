<?php
// dept_admin/maintenance/dashboard.php
// ══════════════════════════════════════════════════════════════════════
...
// ── Stats ──────────────────────────────────────────────────────────────
$stats = [];
$r = $conn->query("SELECT COUNT(*) AS n FROM complaints WHERE dept_id = 2");
$stats['total'] = $r->fetch_assoc()['n'];
$r = $conn->query("SELECT COUNT(*) AS n FROM complaints WHERE dept_id = 2 AND status = 'open'");
$stats['open'] = $r->fetch_assoc()['n'];
$r = $conn->query("SELECT COUNT(*) AS n FROM complaints WHERE dept_id = 2 AND status = 'in_progress'");
$stats['in_progress'] = $r->fetch_assoc()['n'];
$r = $conn->query("SELECT COUNT(*) AS n FROM complaints WHERE dept_id = 2 AND status = 'closed'");
$stats['closed'] = $r->fetch_assoc()['n'];

// Priority breakdown (for chart)
$r = $conn->query("SELECT priority, COUNT(*) AS n FROM complaints WHERE dept_id = 2 GROUP BY priority");
$priorityCounts = ['high'=>0,'medium'=>0,'low'=>0];
while($row = $r->fetch_assoc()) $priorityCounts[$row['priority']] = (int)$row['n'];

// Rating
$r = $conn->query("SELECT ROUND(AVG(f.rating),1) AS avg_r, COUNT(*) AS total_r FROM ticket_feedback f JOIN complaints c ON c.ticket_id=f.ticket_id WHERE c.dept_id=2");
...
$ratingBreakdown = [];
for ($i = 5; $i >= 1; $i--) {
    $r = $conn->query("SELECT COUNT(*) AS n FROM ticket_feedback f JOIN complaints c ON c.ticket_id=f.ticket_id WHERE c.dept_id=2 AND f.rating=$i");
    $ratingBreakdown[$i] = (int)$r->fetch_assoc()['n'];
}

// Category breakdown (for chart)
$catBreakdown = $conn->query("SELECT cat.category_name, COUNT(*) AS n FROM complaints c JOIN categories cat ON cat.category_id=c.category_id WHERE c.dept_id=2 GROUP BY c.category_id ORDER BY n DESC")->fetch_all(MYSQLI_ASSOC);
...
$openTickets = $conn->query("
    ...
    FROM complaints
    WHERE dept_id = 2 AND status = 'open'
...
$topDepts = $conn->query("SELECT c.my_department AS dept_name, COUNT(*) AS n FROM complaints c WHERE c.dept_id=2 GROUP BY c.my_department ORDER BY n DESC LIMIT 5")->fetch_all(MYSQLI_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Maintenance Admin — Dashboard | UniKL Help Desk</title>
...
<main class="main-content">
  ...
  <div class="dash-header">
    <div>
      <div class="dash-eyebrow">Maintenance · Admin Panel</div>
      <h1 class="dash-h1">Maintenance <span class="accent">Dashboard</span></h1>
    </div>
